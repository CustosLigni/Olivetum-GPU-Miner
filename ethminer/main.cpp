...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        app.add_flag("--olivetum", m_PoolSettings.olivetum,
            "Use Olivetumhash RPC (olivetumhash_getWork/submitWork) and Olivetum epochs");
...
#endif
        auto sim_opt =
            app.add_option("-Z,--simulation,-M,--benchmark", m_PoolSettings.benchmarkBlock, "", true);
        auto oliv_bench_opt = app.add_option("--olivetum-benchmark", m_PoolSettings.benchmarkOlivetumEpoch,
            "Benchmark Olivetumhash at given epoch (simulation, no pool)", true);
        ...
        */
        ...
        bool sim_ethash = sim_opt->count();
        bool sim_olivetum = (oliv_bench_opt && oliv_bench_opt->count());
        if (sim_ethash && sim_olivetum)
            throw std::invalid_argument("Choose either --benchmark/-M/-Z or --olivetum-benchmark, not both.");

        if (sim_olivetum)
        {
            m_mode = OperationMode::Simulation;
            pools.clear();
            m_PoolSettings.benchmarkOlivetum = true;
            // Align block to epoch for consistency with epoch calculation
            m_PoolSettings.benchmarkBlock = m_PoolSettings.benchmarkOlivetumEpoch * 11520;
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "test")
        {
            cout << "Benchmarking / Simulation options :" << endl
                 ...
                 << "    --olivetum-benchmark UINT [0 ..] Default not set" << endl
                 << "                        Olivetumhash benchmark at given epoch (no pool)." << endl
                 << endl
