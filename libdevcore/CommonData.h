...
{
    return bytes(reinterpret_cast<::byte const*>(_b.data()),
        reinterpret_cast<::byte const*>(_b.data() + _b.size()));
}
...
inline T fromBigEndian(_In const& _bytes)
{
    T ret = (T)0;
    for (auto i : _bytes)
        ret = (T)((ret << 8) | static_cast<::byte>(
                                  static_cast<typename std::make_unsigned<typename _In::value_type>::type>(i)));
    return ret;
}
