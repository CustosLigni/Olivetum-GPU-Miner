...
SimulateClient::SimulateClient(unsigned const& block, bool olivetum, int olivetumEpoch)
  : PoolClient(), Worker("sim"), m_block(block), m_olivetum(olivetum), m_olivetumEpoch(olivetumEpoch)
{
}
...
void SimulateClient::submitSolution(const Solution& solution)
{
    ...
    bool accepted = false;
    if (solution.work.algo == "olivetumhash")
    {
        auto res = OlivetumhashAux::eval(solution.work.epoch, solution.work.header, solution.nonce);
        accepted = (res.value <= solution.work.boundary);
    }
    else
    {
        auto res = EthashAux::eval(solution.work.epoch, solution.work.header, solution.nonce);
        accepted = (res.value <= solution.work.boundary);
    }
...
void SimulateClient::workLoop()
{
    ...
    if (m_olivetum)
    {
        current.algo = "olivetumhash";
        current.epoch = m_olivetumEpoch;
        // Align block number to epoch if caller didn't provide one.
        if (current.block <= 0)
            current.block = std::max(0, m_olivetumEpoch * 11520);
    }
