...
EthGetworkClient::EthGetworkClient(int worktimeout, unsigned farmRecheckPeriod, bool olivetum)
  : PoolClient(),
    ...
    if (_id == 0 || _id == 1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                WorkPackage newWp;
                newWp.algo = m_olivetum ? "olivetumhash" : "ethash";
                ...
                if (JPrm.size() > 3)
                {
                    std::string blkHex = JPrm.get(Json::Value::ArrayIndex(3), "").asString();
                    try
                    {
                        if (boost::algorithm::istarts_with(blkHex, "0x"))
                            blkHex = blkHex.substr(2);
                        newWp.block = std::stoi(blkHex, nullptr, 16);
                    }
                    catch (const std::exception&)
                    {
                        newWp.block = -1;
                    }
                    if (m_olivetum && newWp.block >= 0)
                        newWp.epoch = newWp.block / 11520;
                }
