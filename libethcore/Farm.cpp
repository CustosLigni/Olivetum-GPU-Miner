...
void Farm::setWork(WorkPackage const& _newWp)
{
    ...
    bool epochChanged = (m_currentWp.epoch != _newWp.epoch) || (m_currentWp.algo != _newWp.algo);
    if (epochChanged)
    {
        if (_newWp.algo == "olivetumhash")
        {
            auto ds = OlivetumhashAux::datasetShared(_newWp.epoch);
            m_currentEc = EpochContext();
            m_currentEc.epochNumber = _newWp.epoch;
            m_currentEc.lightNumItems = 0;
            m_currentEc.lightSize = 0;
            m_currentEc.dagNumItems = 0;
            m_currentEc.dagSize = ds ? ds->size() : 0;
            m_currentEc.lightCache = nullptr;
            m_currentEc.isOlivetum = true;
            m_currentEc.olivetumDataset = ds;
        }
        else
        {
            ...
            m_currentEc.isOlivetum = false;
            m_currentEc.olivetumDataset.reset();
        }
...
void Farm::submitProofAsync(Solution const& _s)
{
    ...
    {
        Result r;
        if (_s.work.algo == "olivetumhash")
        {
            auto orr = OlivetumhashAux::eval(_s.work.epoch, _s.work.header, _s.nonce);
            r.value = orr.value;
            r.mixHash = orr.mixHash;
        }
        else
        {
            r = EthashAux::eval(_s.work.epoch, _s.work.header, _s.nonce);
        }
