...
void CLMiner::workLoop()
{
    ...
    try
    {
        while (!shouldStop())
        {
            ...
            bool isOlivetum = (w.algo == "olivetumhash");

            if (current.header != w.header || current.algo != w.algo)
            {
                ...
                if (m_isOlivetumEpoch)
                {
                    m_queue[0].enqueueWriteBuffer(
                        m_target[0], CL_FALSE, 0, 32, w.boundary.data());
                    m_searchKernel.setArg(0, m_searchBuffer[0]);
                    m_searchKernel.setArg(1, m_header[0]);
                    m_searchKernel.setArg(2, m_dataset[0]);
                    m_searchKernel.setArg(3, m_olivetumChunkCount);
                    m_searchKernel.setArg(5, m_target[0]);
                }
                else
                {
...
bool CLMiner::initEpoch_internal()
{
    ...
    m_isOlivetumEpoch = m_epochContext.isOlivetum;
    ...
    if (m_isOlivetumEpoch)
    {
        m_olivetumDatasetSize =
            m_epochContext.olivetumDataset ? m_epochContext.olivetumDataset->size() : 0;
        if (m_olivetumDatasetSize == 0)
        {
            cllog << "Olivetumhash: missing GPU dataset for epoch " << m_epochContext.epochNumber;
            pause(MinerPauseEnum::PauseDueToInitEpochError);
            return true;
        }
        m_olivetumChunkCount = static_cast<unsigned>(m_olivetumDatasetSize / 64);
        ...
        try
        {
            ...
            string code = string(olivetumhash_cl, olivetumhash_cl + sizeof(olivetumhash_cl));
            addDefinition(code, "WORKSIZE", m_settings.localWorkSize);
            addDefinition(code, "MAX_OUTPUTS", c_maxSearchResults);
            if (!m_settings.noExit)
                addDefinition(code, "FAST_EXIT", 1);

            cllog << "Olivetumhash: compiling OpenCL kernel";
            cl::Program program(m_context[0], code);
            try
            {
                program.build(vector<cl::Device>(&m_device, &m_device + 1));
            }
            catch (cl::Error const&)
            {
                cwarn << "Build log: " << program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(m_device);
                throw;
            }
            ...
            // Buffers
            m_header.clear();
            m_header.emplace_back(m_context[0], CL_MEM_READ_ONLY, 32);

            m_searchBuffer.clear();
            m_searchBuffer.emplace_back(m_context[0], CL_MEM_WRITE_ONLY, sizeof(SearchResults));

            m_dataset.clear();
            m_dataset.emplace_back(m_context[0], CL_MEM_READ_ONLY, m_olivetumDatasetSize);
            m_target.clear();
            m_target.emplace_back(m_context[0], CL_MEM_READ_ONLY, 32);

            m_queue[0].enqueueWriteBuffer(m_dataset[0], CL_TRUE, 0, m_olivetumDatasetSize,
                m_epochContext.olivetumDataset->data());

            // Constant args; variable args (nonce/target) set in workLoop
            m_searchKernel.setArg(0, m_searchBuffer[0]);
            m_searchKernel.setArg(1, m_header[0]);
            m_searchKernel.setArg(2, m_dataset[0]);
            m_searchKernel.setArg(3, m_olivetumChunkCount);
            ...
            cllog << "Olivetumhash: dataset " << dev::getFormattedMemory((double)m_olivetumDatasetSize)
                  << " loaded in "
        ...
        {
            cwarn << ethCLErrorHelper("Olivetumhash OpenCL initialization failed", err);
            pause(MinerPauseEnum::PauseDueToInitEpochError);
            return true;
        }
