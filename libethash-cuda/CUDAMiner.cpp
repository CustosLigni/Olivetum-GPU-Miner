...
bool CUDAMiner::initDevice()
{
    ...
    try
    {
        ...
        CUDA_SAFE_CALL(cudaGetDeviceProperties(&props, m_deviceDescriptor.cuDeviceIndex));
        cudalog << "Built-in CUDA arch targets: " << ETHMINER_CUDA_ARCH_LIST
                << " (max sm_" << ETHMINER_CUDA_MAX_ARCH << "), device sm_" << props.major
                << props.minor;
    }
    catch (const cuda_runtime_error&)
    {
        // If this fails we still proceed; logging only.
    }
...
bool CUDAMiner::initEpoch_internal()
{
    ...
    if (m_epochContext.isOlivetum)
    {
        try
        {
            size_t datasetSize = m_epochContext.olivetumDataset ? m_epochContext.olivetumDataset->size() : 0;
            if (datasetSize == 0)
            {
                cudalog << "Olivetumhash dataset unavailable for epoch " << m_epochContext.epochNumber;
                pause(MinerPauseEnum::PauseDueToInitEpochError);
                return true;
            }

            // Allocate buffers if size changed
            if (m_allocated_olivetum_dataset < datasetSize)
            {
                CUDA_SAFE_CALL(cudaDeviceReset());
                CUDA_SAFE_CALL(cudaSetDeviceFlags(m_settings.schedule));
                CUDA_SAFE_CALL(cudaDeviceSetCacheConfig(cudaFuncCachePreferL1));

                CUDA_SAFE_CALL(cudaMalloc(reinterpret_cast<void**>(&m_olivetum_dataset_dev), datasetSize));
                m_allocated_olivetum_dataset = datasetSize;
            ...
            CUDA_SAFE_CALL(cudaMemcpy(reinterpret_cast<void*>(m_olivetum_dataset_dev),
                m_epochContext.olivetumDataset->data(), datasetSize, cudaMemcpyHostToDevice));
            set_olivetum_dataset(m_olivetum_dataset_dev, datasetSize);

            cudalog << "Uploaded Olivetumhash dataset (" << dev::getFormattedMemory((double)datasetSize)
                    << ") to GPU in "
...
void CUDAMiner::workLoop()
{
    ...
    try
    {
        while (!shouldStop())
        {
            ...
            // Epoch change ?
            if (current.epoch != w.epoch || current.algo != w.algo)
            {
                if (!initEpoch())
...
{
    if (w.algo == "olivetumhash")
    {
        if (!m_olivetum_dataset_dev || m_allocated_olivetum_dataset == 0)
        {
            cudalog << "No Olivetumhash dataset on GPU; pausing miner";
            pause(MinerPauseEnum::PauseDueToInitEpochError);
            return;
        }

        set_olivetum_header(header);
        set_olivetum_target(w.boundary.data());
             ...
             current_index++, start_nonce += m_batch_size)
        {
            ...
            buffer.count = 0;
            run_olivetumhash_search(
                m_settings.gridSize, m_settings.blockSize, stream, &buffer, start_nonce);
        }

        // Catch invalid device function / launch errors early (e.g. missing SASS for older GPUs).
        cudaError_t launchErr = cudaPeekAtLastError();
        if (launchErr != cudaSuccess)
        {
            cudalog << "CUDA launch error: " << cudaGetErrorString(launchErr);
            pause(MinerPauseEnum::PauseDueToInitEpochError);
            return;
        }
        ...
        while (!done)
        {
                 ...
                 current_index++, start_nonce += m_batch_size)
            {
                ...
                if (!done)
                    run_olivetumhash_search(m_settings.gridSize, m_settings.blockSize, stream,
                        &buffer, start_nonce);
            ...
            // Update hash rate every iteration of the loop
